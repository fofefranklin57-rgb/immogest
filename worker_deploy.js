...
${_footer()}</body></html>`;
    ...
    try {
      ...
      if (path === '/db' && request.method === 'POST') {
        ...
        const { table, data, tenantId, userId } = body;
        ...
        const ROLE_LEVEL = { admin:6, coordinateur:5, gestionnaire:4, comptable:3, agent:2, bailleur:2, locataire:1 };

        // Valider userId + extraire rôle réel
        let userRole = 'admin';
        if (userId && userId !== 'admin_' + tenantId) {
          const uRes = await sbFetch('users_app', '?id=eq.' + encodeURIComponent(userId) + '&tenant_id=eq.' + tenantId + '&select=id,role,actif&limit=1');
          const uData = uRes.ok ? await uRes.json() : [];
          if (!uData.length) return json({ error: 'Session invalide' }, 401);
          if (uData[0].actif === false) return json({ error: 'Compte désactivé' }, 403);
          userRole = uData[0].role || 'agent';
        } else {
          // Pas de userId ou admin_ synthétique : vérifier le tenant existe
        ...
        if (action === 'delete' && roleLevel < 4)
          return json({ error: 'Permission insuffisante pour supprimer' }, 403);
        if ((action === 'update' || action === 'patch' || action === 'upsert') && roleLevel < 3)
          return json({ error: 'Permission insuffisante pour modifier' }, 403);
        if (action === 'insert' && roleLevel < 2)
          return json({ error: 'Permission insuffisante' }, 403);

        // Locataire : lecture seule sur ses propres données
        if (userRole === 'locataire' && action !== 'select')
          return json({ error: 'Permission insuffisante' }, 403);
        ...
        if (method === 'DELETE') {
          logEvent(tenantId, 'warn', 'db.delete', 'Suppression ' + table, { userId, table, filters });
          return json({ success: true, data: [], ok: true });
        }
        const result = await res.json();
        if (!res.ok) return json({ error: 'Erreur base de données', detail: result }, 500);
        if (method !== 'GET') logEvent(tenantId, 'info', 'db.' + action, action + ' ' + table, { userId, table });
      ...
      if (path === '/upload' && request.method === 'POST') {
        ...
        const tRes = await sbFetch('tenants', '?id=eq.' + tenantId + '&select=id');
        const tData = tRes.ok ? await tRes.json() : [];
        if (!tData.length) return json({ error: 'Non autorisé' }, 401);
